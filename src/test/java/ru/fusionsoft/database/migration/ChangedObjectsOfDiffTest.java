...
import ru.fusionsoft.database.diff.objects.ChangedObjectsOfDiff;
...
class ChangedObjectsOfDiffTest {

    @Test
    public void dosome() {
        final String target = "dvdrental";
        final ChangedObjectsOfDiff<YamlNode> changed = new ChangedObjectsOfDiff<>(
        ...
        for (final Map.Entry<ObjectSignature, TemporalDiff<DbObject<YamlNode>>> entry : changed.entrySet()) {
            Assertions.assertNotEquals(
                entry.getValue().current().asYaml().toString(),
