...
/**
 * Migrations to drop views, functions, triggers, that are in current database state.
 * @since 0.1
 */
public class SqlsDropCurrentMigrations extends IterableOfScalarSticky<Migration> {

    /**
     * Instantiates a new Sqls drop migrations.
     ...
     */
    public SqlsDropCurrentMigrations(
                ...
                return new Mapped<>(
                    ...
                    new Reversed<>(
                        new ObjectsSortedByTypeAndDependencies(
                            new ObjectsWithSqlOrigin(diff.current())
                        )
                    )
                );
            }
