...
import ru.fusionsoft.database.migration.common.ConstraintCreateMigration;
...
/**
 * Migrations to create all constraints that exist in 'next' state.
 * @since 0.1
 * @checkstyle ClassDataAbstractionCouplingCheck (200 lines)
 */
public class ConstraintsCreateNewMigrations extends IterableOfScalarSticky<Migration> {
     ...
     */
    public ConstraintsCreateNewMigrations(
        final TemporalDiff<Iterable<DbObject<YamlNode>>> diff,
        final Dbms dbms
    ) {
        super(
            () -> {
                final Iterable<DbObject<YamlNode>> all = diff.next();
                final Set<String> partitions = new SetOf<>(
                    new Mapped<String>(
                        ot -> ot.signature().name().asString(),
                        new Filtered<>(
                            obj -> new And(
                                () -> obj.signature().type().equalsTo(new ObjectTypeTable()),
                ...
                return new Mapped<>(
                    con -> new ConstraintCreateMigration(con, dbms),
                    new Joined<>(
                        new Filtered<>(
                            con -> new Not(
