...
import ru.fusionsoft.database.migration.postgres.PgIndexCreateMigration;
import ru.fusionsoft.database.snapshot.DbObject;
import ru.fusionsoft.database.snapshot.Dbms;
import ru.fusionsoft.database.snapshot.dbms.PostgresDbms;
import ru.fusionsoft.lib.collection.StrictMap;

public class IndexCreateMigration extends MigrationOfScalarSticky {

    public IndexCreateMigration(final DbObject<DbdIndexMapping> constraint, final Dbms dbms) {
        super(
            () -> {
                return new StrictMap<>(
                    new MapOf<>(
                        new MapEntry<Dbms, Scalar<Migration>>(
                            new PostgresDbms(),
                            () -> new PgIndexCreateMigration(constraint)
                        )
                    )
                ).get(dbms).value();
            }
