...
public class PgConstraintCreateMigration implements Migration {

    private final ObjectSignature signature;

    private final Text ddl;

    private PgConstraintCreateMigration(final ObjectSignature signature, final Text ddl) {
        this.signature = signature;
        this.ddl = ddl;
    }

    public PgConstraintCreateMigration(final DbObject<DbdConstraintMapping> constraint) {
        this(
            ...
            new TextOfScalar(
                new Ternary<String>(
                    ...
                    () -> new TextOfMessageFormat(
                        ...
                        () -> new TextOfObjectField(constraint, DbdConstraintFields.DDL)
                    ).asString()
                )
            )
        );
    ...
    public final Text description() {
        return new TextOfMessageFormat(
            "Creating constraint {1} of table {0}",
