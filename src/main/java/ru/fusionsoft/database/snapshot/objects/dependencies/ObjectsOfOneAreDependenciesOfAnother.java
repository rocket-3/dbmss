 ...
 * @since 0.1
 */
public class ObjectsOfOneAreDependenciesOfAnother<T extends YamlNode> extends ObjectsOfScalar<T> {

    /**
     * Instantiates a new Objects of one are dependencies of another.
     * @param ones The objects to take from.
     * @param anothers The objects that could depend on first ones.
     * @param <Y> The type of YamlNode parameter
     */
    public <Y extends YamlNode> ObjectsOfOneAreDependenciesOfAnother(
        final Iterable<DbObject<T>> ones,
        final Iterable<DbObject<Y>> anothers
    ) {
        super(
            () -> {
                final Sticky<ObjectDependsOnAnotherPredicate<T>> predicates = new Sticky<>(
                    new Mapped<>(
                        ObjectDependsOnAnotherPredicate::new,
                        anothers
                    )
                );
                return new ObjectsFiltered<T>(
                    ones,
                    (DbObject<T> one) -> new Or(
                        (ObjectDependsOnAnotherPredicate<T> predicate) -> predicate.apply(one),
                        predicates
                    ).value()
                );
            }
