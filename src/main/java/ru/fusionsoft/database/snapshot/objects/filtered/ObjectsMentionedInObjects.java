...
/**
 * The {@link ObjectsFiltered} by being mentioned in another {@link Iterable} of {@link DbObject}s.
 * @param <T> The type of {@link YamlNode} parameter.
 * @since 0.1
 */
public class ObjectsMentionedInObjects<T extends YamlNode> extends IterableEnvelope<DbObject<T>> {
     ...
     */
    public <Y extends YamlNode> ObjectsMentionedInObjects(
        final Iterable<DbObject<Y>> mentions,
        final Iterable<DbObject<T>> origin
    ) {
        super(
            new Filtered<>(
                new ObjectMentionedInObjectsPredicate(mentions),
                origin
            )
        );
    }
